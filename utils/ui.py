...
def navigation_buttons(disable_navigation_dict):
    ...
    with col1: st.page_link("1_Home.py", label="HOME", icon="🏠", disabled=disable_navigation_dict["HOME"])
    with col2: st.page_link(os.path.join("pages","2_Login.py"), label="LOGIN", icon="1️⃣", disabled=disable_navigation_dict["LOGIN"])
    with col3: st.page_link(os.path.join("pages","3_Files.py"), label="FILES", icon="2️⃣", disabled=disable_navigation_dict["FILES"])
    with col4: st.page_link(os.path.join("pages","4_Chat.py"), label="CHAT", icon="3️⃣", disabled=disable_navigation_dict["CHAT"])
    with col5: st.page_link(os.path.join("pages","5_Image.py"), label="IMAGE CHAT", icon="4️⃣", disabled=disable_navigation_dict["IMAGE"])
...
def fileSelectorUI(path):
    ...
    options = st.multiselect("Please Select A File to Proceed.", set(valid_files), placeholder="Please select a file!")
    st.session_state['files'] = list(options)
