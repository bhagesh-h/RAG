...
def create_embeddings(text_contents, embedding_model, base_url):
    """
    Creates embeddings for the given text contents using the specified embedding model.
    """
...
def create_bm25_retriever(text_contents):
    """
    Creates a BM25 retriever for the given text contents.
    """
...
def perform_hybrid_retrieval(query, vector_store, bm25_retriever):
    """
    Performs hybrid retrieval by combining results from BM25 and FAISS.
    """
    # Use the updated `invoke` method for BM25 retriever
    bm25_results = bm25_retriever.invoke(query)
    faiss_results = vector_store.similarity_search(query, k=5)
    combined_results = bm25_results + faiss_results
    return combined_results[:5]  # Limit to top 5 results

def process_text_and_retrieve(text_contents, query, embedding_model, base_url, num_results=3):
    """
    Orchestrates the hybrid retrieval pipeline by calling individual task functions.
    Retrieves only the top 5 lines from the ranked text content.
    """
