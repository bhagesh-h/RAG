...
def fileSelectorUI(path=os.getcwd()):
    ...
    st.write("Following Files will be used as input for QnA:", options)
    st.session_state['files'] = options
    if st.button("Submit"):
        st.switch_page("pages/CHAT.py")

def imageSelectorUI(path=os.getcwd()):
    ...
    try:
        ...
        valid_files = path
    img = image_select("Label", valid_files)
    st.session_state['image'] = img

def chatUI(models,client=''):
